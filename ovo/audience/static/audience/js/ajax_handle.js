...
function add_new_video(){
	...
	$.ajax({
	    ...
	    success: function(data) {
			...
			video_html.push('<a class="deleteVideo" onclick="delete_video(\'addedVideo-' + video_count.toString() +'\', \'video-preview-' + video_count.toString() +'\', \'-1\')">&mdash;</a></div><div id="video_' + video_count.toString() +'" class="videoitem col-md-1">');
			...
			var options_html = [];
			for(i=0;i<main_data.data.length;i++){
			...
			video_html.push('<br><br><div class="video col-md-4_7"><img src="/static/company/images/videopreview.jpg" id="img-preview-' + video_count.toString() +'">');
		    ...
		    video_html.push('<iframe id="youtube-preview-' + video_count.toString() +'" width="530" height="316" frameborder="0" allowfullscreen style="display:none"></iframe>');
			...
			video_html.push('<input onclick="check_and_submit()" class="save" value="Save" readonly="true"></div></div></div></div><!--end video-->');

			var final_video_html = video_html.join('');
			$('#addedVideos').prepend(final_video_html);
...
function delete_video(mainVideoDiv, videoElement, video_id){
	video_count = video_count -1;
	var video_path = $('#'+videoElement).attr('src');
	$('#'+mainVideoDiv).remove();
	ajax_request("delete_video", 'empty', {data: "{'video_path': '" + video_path + "', 'video_id': '" + video_id + "'}"});
}
